...
const OFF=new Set(["_missing","analyst"]);  // 기본 꺼짐(잡음): 미생성·애널
const active=new Set(present.filter(t=>!OFF.has(t)));
...
present.forEach(t=>{
  ...
  l.innerHTML=`<input type="checkbox" ${active.has(t)?"checked":""} data-t="${t}"><i style="background:${TC[t]||'#999'}"></i>${TYPE_LABEL[t]||t}<span class="cnt">${typeCount[t]||0}</span>`;
...
applyFilter();  // 초기 필터 적용(미생성·애널 기본 숨김)
