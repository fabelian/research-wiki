...
8) `## 주요 이벤트`(또는 타임라인) — 반복 서술을 사건당 1개 한 줄로 압축(날짜+출처). **최신 날짜가 맨 위로 오도록 내림차순 정렬**
9) `## 관련 출처/링크`
10) 🐦SNS 라벨 블록이 있으면 라벨과 함께 보존

[정렬 규칙] 날짜로 식별되는 섹션·항목(예: "## 2026-06-04 업데이트", "## YYYY-MM-DD ...", 히스토리 표의 행, 타임라인 불릿)은 모두 **최신 날짜가 위로 오는 내림차순**으로 정렬한다.
