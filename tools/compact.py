#!/usr/bin/env python
"""
compact.py — 비대해진 위키 페이지를 강력 모델(Claude Opus)로 정기 압축한다.
...
의존성: anthropic  (pip install -r tools/requirements.txt)
"""
import argparse
import glob
import os
...
MODEL = os.environ.get("COMPACT_MODEL", "claude-opus-4-8")
...
def main():
    ...
    try:
        ...
        sys.exit("[ERR] pip install -r tools/requirements.txt 필요(anthropic)")
